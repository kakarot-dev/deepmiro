...
class ReportManager:
    ...
    def get_report_by_simulation(cls, simulation_id: str) -> Optional[Report]:
        """Return the MOST RECENT report for a simulation.

        A simulation accumulates a new report on every force_regenerate.
        The old implementation returned the first os.listdir() match —
        arbitrary filesystem order — so a stale earlier report could win
        over a freshly regenerated one. list_reports() already sorts by
        created_at descending; take its first entry.
        """
