...
Provides: CRUD, NER/RE-based text ingestion, hybrid search, retry logic,
and graph reasoning queries (centrality, communities, paths, contradictions).
"""
...
class Neo4jStorage(GraphStorage):
    ...
    def add_text(self, graph_id: str, text: str) -> str:
        ...
        logger.info(
            f"[add_text] Starting NER extraction for chunk ({len(text)} chars)..."
        )
        ...
        fact_texts = [
            r.get("fact", f"{r['source']} {r['type']} {r['target']}") for r in relations
        ]
        ...
        if all_texts_to_embed:
            logger.info(
                f"[add_text] Batch-embedding {len(all_texts_to_embed)} texts..."
            )
            try:
                all_embeddings = self._embedding.embed_batch(all_texts_to_embed)
            except Exception as e:
                logger.warning(
                    f"[add_text] Batch embedding failed, falling back to empty: {e}"
                )
        ...
        with self._driver.session() as session:
            ...
            for idx, entity in enumerate(entities):
                ...
                embedding = (
                    entity_embeddings[idx] if idx < len(entity_embeddings) else []
                )
            ...
            if entity_batch:
                ...
                uuid_pairs = self._call_with_retry(
                    session.execute_write, _merge_entities_batch
                )
    ...
    def get_all_nodes(self, graph_id: str, limit: int = 2000) -> List[Dict[str, Any]]:
        def _read(tx):
            ...
            return [
                self._node_to_dict(record["n"], record["labels"]) for record in result
            ]
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """O(1) Cypher — direct index lookup, not a full scan."""
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        ...
        def _read(tx):
            ...
            # Get all edges with source/target node names
            edge_result = tx.run(
                """
                MATCH (src:Entity)-[r:RELATION {graph_id: $gid}]->(tgt:Entity)
                RETURN r, src.uuid AS src_uuid, tgt.uuid AS tgt_uuid,
    ...
    def get_bridge_entities(
        ...
        try:
            ...
            logger.warning(
                f"Bridge entity query failed (may need Neo4j 5.9+ for EXISTS subquery): {e}"
            )
    ...
    def get_temporal_evolution(
        ...
        # Group into time buckets (by date portion)
