"""
EmbeddingService — pluggable embedding via OpenAI-compatible API or Ollama.

Supports providers (set via EMBEDDING_PROVIDER env var):
  - "openai"  (default): Uses /v1/embeddings endpoint
                          Works with Fireworks AI, OpenRouter, OpenAI, Azure, etc.
  - "ollama":            Uses Ollama's /api/embed endpoint (local)
...
class EmbeddingService:
    """Generate embeddings using an OpenAI-compatible or Ollama API."""
    ...
    def __init__(
        ...
        if self.provider == "ollama":
            ...
            # OpenAI-compatible (Fireworks, OpenRouter, OpenAI, etc.)
    ...
    def _request_openai(self, texts: List[str]) -> List[List[float]]:
        """OpenAI-compatible /v1/embeddings endpoint (Fireworks, OpenRouter, etc.)."""
        payload: dict = {
            "model": self.model,
            "input": texts,
        }
    ...
    def _do_request(self, payload: dict, parser) -> List[List[float]]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                last_error = e
                logger.error(
                    f"Embedding HTTP error: {e.response.status_code} - {e.response.text}"
                )
            ...
            if attempt < self.max_retries - 1:
                wait = 2**attempt
                logger.info(f"Retrying embedding in {wait}s...")
                time.sleep(wait)
